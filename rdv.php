	...
	<!-- start: Header -->
		...
		<div class="row-fluid">
			...
			<ul class="breadcrumb">
				...
				<li><a href="rdv.php">RDV Fixed</a></li>
			</ul>
			...
			<div class="row-fluid sortable">
				<div class="box span12">
					<div class="box-header" data-original-title>
						<h2><i class="halflings-icon white user"></i><span class="break"></span>RDV Fixed</h2>
					</div>
					<div class="box-content">
						<div class="row-fluid">
                        	<div class="x-box span12">
                                	...
                                	<form action="" name="search-client" method="post" enctype="multipart/form-data" class="form-horizontal">
                                    	<fieldset>
                                        	<div class="control-group">
                                            	<label class="control-label" for="keyword">Search RDV: </label>
                                                <div class="controls">
                                                	<?php 
                                ...
                                <?php 
								$rdv_sql = "SELECT a.rdv, a.lawyer_id, b.name AS client_name, b.phone FROM `case_info` a LEFT JOIN `clients` b ON a.client_id=b.id LEFT JOIN `users` c ON a.lawyer_id=c.id WHERE a.status=1 AND c.status=1 ";
								if($userGroup!=1){
									$rdv_sql .= " AND a.lawyer_id=$lawyer_id";
								}
								if($keyword){
									$rdv_date = date('Y-m-d', strtotime($keyword));
									$rdv_sql .= " AND a.rdv='$rdv_date'";
								}
								$rdv_sql .= " ORDER BY a.rdv ASC";
								
								$client_query = $lawyer->db->query($rdv_sql);
								?>
								<?php 
                                if($client_query->num_rows){
                                    ...
                                    <div class="box">
                                        ...
                                        <div class="row-fluid">
                                            ...
                                            <div class="span8"><?php echo ($client->rdv=='0000-00-00') ? '-' : date('m/d/Y', strtotime($client->rdv)); ?></div> 
                                        </div>
                                        </div>
                                    </div>
                                <?php 
